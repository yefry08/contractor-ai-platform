...
| República Dominicana | DGCP | ⚠️ API real (CKAN en datos.gob.do) pero los archivos están bloqueados para clientes automatizados | `datos.gob.do` (portal CKAN oficial) responde bien vía `curl` para metadata (`package_search`, `package_show`) — confirmado con 6 datasets de la DGCP. Pero ninguno de esos datasets está en el datastore consultable de CKAN (`datastore_active` = false en los 6); todos apuntan a archivos estáticos alojados en `dgcp.gob.do`, que está detrás de Cloudflare con protección anti-bot que devuelve 403 a `curl` de forma inmediata y consistente — mientras que el mismo archivo se descarga sin problema desde un navegador real (verificado). No se intentó eludir la protección (headers falsos, etc.) — es una decisión deliberada de anti-abuso del lado de DGCP, no un bug a explotar. |
